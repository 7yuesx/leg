...
class PIDController:
    def __init__(self, kp, ki, kd, output_min=float('-inf'), output_max=float('inf')):
        ...
        :param output_min: 输出限制最小值 (防止电机反转过快)
        :param output_max: 输出限制最大值 (防止PWM过大烧毁电机)
        """
        self.kp = kp
        self.ki = ki
        self.kd = kd
        ...
        self.integral = 0.0    # 积分累计值
        self.last_time = None  # 上一次计算的时间戳

    def update(self, target, current, dt=None):
        """
        计算 PID 输出
        :param target: 目标值 (例如：目标角度)
        :param current: 当前值 (例如：传感器读回的角度)
        :param dt: 循环时间间隔 (秒)。如果为None，则自动计算。
        :return: 控制量 (例如：电机速度/扭矩/PWM)
        """
        ...
        # 积分限幅 (抗积分饱和 Anti-windup)：防止误差累积过大导致超调
        # 这里简单处理：如果设定了输出限制，也限制一下积分项（可选优化）
